...
if st.button('실행'):
    ...
    selected_columns['SUM'] = selected_columns['PER 순위'] + selected_columns['ROE 순위']

    # 'MAGIC SUM' 오름차순으로 순위 부여
    selected_columns['Ranking'] = selected_columns['SUM'].rank(method='min', ascending=True)

    # 'MAGIC 순위' 기준 오름차순으로 정렬
    selected_columns_sorted = selected_columns.sort_values(by='Ranking', ascending=True)
